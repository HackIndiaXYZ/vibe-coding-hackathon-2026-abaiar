...
import { TTS_PROVIDERS, getTTSProvider } from '@/lib/audio/constants';
import type { TTSProviderId } from '@/lib/audio/types';
import { ASRSettings } from './asr-settings';
import { ASR_PROVIDERS, getASRProvider } from '@/lib/audio/constants';
...
export function SettingsDialog({ open, onOpenChange, initialSection }: SettingsDialogProps) {
  ...
  const getHeaderContent = () => {
    switch (activeSection) {
      ...
      case 'tts': {
        const ttsIcon = getTTSProvider(ttsProviderId)?.icon;
        return (
          <>
            {ttsIcon ? (
              <img
      ...
      case 'asr': {
        const asrIcon = getASRProvider(asrProviderId)?.icon;
        return (
          <>
            {asrIcon ? (
              <img
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="h-[85vh] p-0 gap-0 block" showCloseButton={false}>
        ...
        <div className="bg-gradient-to-r from-purple-600/20 to-indigo-600/20 border-b border-purple-500/30 px-4 py-3 flex items-center gap-3">
          <svg className="h-5 w-5 text-purple-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
