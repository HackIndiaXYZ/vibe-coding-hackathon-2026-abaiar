...
export async function generateTTS(
  ...
  if (provider?.requiresApiKey && !config.apiKey) {
    throw new Error(
      `语音合成失败：${config.providerId} 未配置 API Key。请在「API 统一管理」页面完成配置。`,
    );
  }
...
async function generateQwenTTS(config: TTSModelConfig, text: string): Promise<TTSGenerationResult> {
  let baseUrl: string = config.baseUrl || TTS_PROVIDERS['qwen-tts'].defaultBaseUrl || 'https://dashscope.aliyuncs.com/api/v1';

  if (baseUrl.includes('/compatible-mode/')) {
    baseUrl = baseUrl.replace(/\/compatible-mode\/v\d+/, '/api/v1');
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Qwen TTS API error${statusInfo}: ${errorText || response.statusText || 'Unknown error'}`,
    );
  }
  ...
  if (data.code && data.code !== '200' && data.code !== 200) {
    throw new Error(
      `Qwen TTS API error: ${data.message || data.msg || JSON.stringify(data)}`,
    );
  }
