          ...
          if (actionsResult.success && actionsResult.scene) {
            ...
            if (settings.ttsEnabled && settings.ttsProviderId !== 'browser-native-tts') {
              ...
              if (!ttsResult.success) {
                log.warn(`TTS partially failed for scene "${outline.title}": ${ttsResult.failedCount} actions failed`, ttsResult.error);
              }
            }
    ...
    async (outlineId: string) => {
      ...
      try {
        ...
        if (settings.ttsEnabled && settings.ttsProviderId !== 'browser-native-tts') {
          ...
          if (!ttsResult.success) {
            log.warn(`TTS partially failed on retry for "${outline.title}": ${ttsResult.failedCount} actions failed`, ttsResult.error);
          }
        }
